...
module.exports = {
  ...
  delete: async (req) => {
    const codeHash = await dashboard.StorageObject.getProperty(`${req.appid}/${req.query.codeid}`, 'codeHash')
    await dashboard.Storage.deleteFile(`${req.appid}/${req.query.codeid}`)
    await dashboard.StorageObject.setProperty(`${req.appid}/${req.account.accountid}`, 'resetCodeLastDeleted', dashboard.Timestamp.now)
