...
module.exports = {
  ...
  patch: async (req) => {
    ...
    const tokenHash = await dashboard.Hash.fixedSaltHash(`${req.data.account.accountid}/${token}/${req.data.account.sessionKey}/${dashboardSessionKey}`, req.alternativeFixedSalt, req.alternativeDashboardEncryptionKey)
    const sessionInfo = {
      object: 'session',
      sessionid: sessionid,
